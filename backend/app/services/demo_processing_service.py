...
LEASE_SECONDS = 300
SAFE_ERROR_MESSAGE = "The demo processing job could not be completed."


class DemoProcessingError(RuntimeError):
...
def _context(session: Session, job_id: int):
    ...
    if row is None:
        raise DemoProcessingError("PROCESSING_CONTEXT_NOT_FOUND")
    return row
...
def _heartbeat(
    ...
    *,
    ...
    if cancellation_requested:
        ...
        raise DemoProcessingError("PROCESSING_CANCELLED")
...
def _registered_artifact(
    ...
    *,
    ...
    if (
        ...
        raise DemoProcessingError("ORIGINAL_INTEGRITY_FAILED")
    digest = sha256()
...
def _decode_rgb(content: bytes) -> np.ndarray:
    try:
        ...
        raise DemoProcessingError("NORMALIZED_IMAGE_INVALID") from None
    return rgb
    ...
    try:
        ...
        try:
            finish_processing_attempt(
                ...
                worker_identity=worker_identity,
                outcome="failed",
                failure_code="DEMO_PROCESSING_FAILED",
            )
        except Exception:
            pass
        raise DemoProcessingError("DEMO_PROCESSING_FAILED") from None
